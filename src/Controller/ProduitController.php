...
class ProduitController extends AbstractController
{
    ...
    public function index(ProduitRepository $produitRepository, Request $request): Response
    {
        ...
        $recherche = $this->createFormBuilder()
        ...
        if ($recherche->isSubmitted() && $recherche->isValid() && !$recherche->get('recherche')->getData() == null) {
            $produit=[];
            $query = $recherche->get('recherche')->getData();
            $resultatID = $produitRepository->chercheProduit($query);

            foreach ($resultatID as $iD) {
                $resultat = $produitRepository->findOneBy(["id" => $iD]);
                array_push($produit,$resultat);
            }
